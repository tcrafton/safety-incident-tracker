...
const initialState = {
  ...
  TEMPORARY_CONTROL: '',
  DATE_TEMPORARY_CONTROL: '',
  PERMANENT_CONTROL: '',
  DATE_PERMANENT_CONTROL: '',
  TYPEOFACCIDENT: 'Hazard Cond',
  ENTEREDBY: 'WEBAPP',
  RCFA_REQUIRED: 1,
};
...
const HazardConditionPage = () => {
  ...
  const [temporaryControlDate, setTemporaryControlDate] = useState(null);
  const [permanentControlDate, setPermanentControlDate] = useState(null);
  ...
  const handleInputChange = (e, param) => {
    if (param === 'ACCIDENTTIME') {
      ...
      setAccidentDate(e);
    } else if (param === 'DATE_TEMPORARY_CONTROL') {
      setAccidentData({ ...accidentData, [param]: e.toLocaleDateString() });
      setTemporaryControlDate(e);
    } else if (param === 'DATE_PERMANENT_CONTROL') {
      setAccidentData({ ...accidentData, [param]: e.toLocaleDateString() });
      setPermanentControlDate(e);
    ...
    axios
      ...
      .then(function (response) {
        if (response.status === 200 || response.status === 204) {
          ...
          setPermanentControlDate(null);
          setTemporaryControlDate(null);
  ...
  return (
    <>
      <Wrapper>
        <h4 className="pageHeader">Hazard Condition Entry</h4>
        ...
        <div>
          <Box>
            <Grid container justify="space-around" spacing={2}>
              ...
              <Grid item xs={2}>
                <InputLabel>Shift Hazard Condition Found On</InputLabel>
                <FormControl sx={{ minWidth: 120 }} size="small">
                  <TextField
                    style={{ width: '100%' }}
                    variant="outlined"
              ...
              <Grid item xs={2}>
                <InputLabel>Hazard Condition Date</InputLabel>
                <DatePicker
              ...
              <Grid item xs={3}>
                <InputLabel>Hazard Condition Time</InputLabel>
                <DatePicker
                  className="datePicker"
              ...
              <Grid item xs={5}>
                <InputLabel>Hazard Condition Location</InputLabel>
                <Autocomplete
                  sx={{
                    maxWidth: '50%',
                  }}
              ...
              <Grid item xs={12}>
                <InputLabel>Nature of Hazard Condition</InputLabel>
                <TextareaAutosize
                  minRows={5}
              ...
              <Box width="100%" />
              <Grid item xs={3}>
                <InputLabel>Temporary Control</InputLabel>
                <TextareaAutosize
                  minRows={5}
                  aria-label="empty textarea"
                  value={accidentData.TEMPORARY_CONTROL}
                  style={{
                    width: 450,
                    padding: '3px',
                    fontSize: '16px',
                  ...
                  onChange={(e, label) =>
                    handleInputChange(e, 'TEMPORARY_CONTROL')
                  }
                />
              </Grid>
              <Grid item xs={2}>
                <InputLabel>Date of Temporary Control</InputLabel>
                <DatePicker
                  className="datePicker"
                  selected={temporaryControlDate}
                  dateFormat="MM/dd/yyyy"
                  onChange={(e, label) =>
                    handleInputChange(e, 'DATE_TEMPORARY_CONTROL')
                  }
                />
              </Grid>
              <Grid item xs={3}>
                <InputLabel>Permanent Control</InputLabel>
                <TextareaAutosize
                  minRows={5}
                  aria-label="empty textarea"
                  value={accidentData.PERMANENT_CONTROL}
                  style={{
                    width: 450,
                    padding: '3px',
                    fontSize: '16px',
                  ...
                  onChange={(e, label) =>
                    handleInputChange(e, 'PERMANENT_CONTROL')
                  }
                />
              </Grid>
              <Grid item xs={2}>
                <InputLabel>Date of Permanent Control</InputLabel>
                <DatePicker
                  className="datePicker"
                  selected={permanentControlDate}
                  dateFormat="MM/dd/yyyy"
                  onChange={(e, label) =>
                    handleInputChange(e, 'DATE_PERMANENT_CONTROL')
                  }
                />
              </Grid>
              <Grid item xs={8}>
